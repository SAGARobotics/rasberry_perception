...
class RunClientOnTopic:
    ...
    def publish_detections(self, image_msg, image_info,depth_msg=None, depth_info=None,  response=None ):
        ...
        if not len(results.objects):
            ...
            return
        # Check if berry is pickable for gripper perception
        if self._service_name == 'gripper_perception':
            results.gripper.single_pickable_berry = False
            no_of_berries = 0
            ...
            for i in range(len(results.objects)):
                ...
                if results.objects[i].class_name =='flesh_unripe':
                    no_of_berries = no_of_berries + 1
            results.gripper.no_of_berries = no_of_berries
            if no_of_ripe_berries == 1:
                results.gripper.single_pickable_berry = True
            else:
                results.gripper.single_pickable_berry = False
